...
class RNN(torch.nn.Module):
    ...
    def __init__(self, n_input, n_output, n_hidden, n_rnn_layer=2, bidirectional=True, dropout=0.2):
        ...
        self.rnn = torch.nn.LSTM(n_hidden, n_hidden, n_rnn_layer, bidirectional=bidirectional, batch_first = True) # batch_first added
    ...
    def forward(
        ...
        :param h: Optional hidden state and cell state (Tuple[Tensor, Tensor])
...
class TransPoseNet(torch.nn.Module):
    ...
    def __init__(self, num_past_frame=20, num_future_frame=5, hip_length=None, upper_leg_length=None,
                 lower_leg_length=None, prob_threshold=(0.5, 0.9), gravity_velocity=-0.018, is_training = False):
        r"""
        ...
        # add info about user's lower body if available
        m = art.ParametricModel(paths.smpl_file)
        j, _ = m.get_zero_pose_joint_and_vertex() # function returns joint vector [batch_size, num_joint, 3] and vertex vector [batch_size, num_vertex, 3]
        b = art.math.joint_position_to_bone_vector(j[joint_set.lower_body].unsqueeze(0), # joint locations of all lower body joints
                                                   joint_set.lower_body_parent).squeeze(0) # parent joints of each joint
                                                    # bone vectors are position differences between a child and parent bone
                                                    # [num_joint = 9, 3]
        bone_orientation, bone_length = art.math.normalize_tensor(b, return_norm=True)
        # print(bone_orientation.shape, bone_length.shape)
        ...
        b[:3] = 0
        # constant
        self.global_to_local_pose = m.inverse_kinematics_R # converts rotations in global frame to rotations in local frame
        self.lower_body_bone = b # [9, 3]
        ...
        if not is_training:
            ...
            # self.load_state_dict(torch.load('logs/240529-212553/weights_best_147.pt'))
            self.eval()
    ...
    def _reduced_glb_6d_to_full_local_mat(self, root_rotation, glb_reduced_pose): 
        ...
        # pose[:, joint_set.ignored] = torch.eye(3, device=pose.device) # 여기서 문제 발생.. (Constant folding in symbolic shape inference fails: shape '[1, 9, 3, 3]' is invalid for input of size 9)
        pose[:, joint_set.ignored] = torch.eye(3, device=pose.device).repeat(joint_set.n_ignored, 1).view(joint_set.n_ignored, 3, 3)
        pose[:, 0] = root_rotation.view(-1, 3, 3)
        return pose
    
    def _prob_to_weight(self, p): # weight used for translation estimation
    ...
    def forward_rnn(self, imu, rnn_state: Optional[Tuple[torch.Tensor, torch.Tensor]] = None):
    ...
    def forward_offline(self, imu):
        ...
        """
        _, _, global_reduced_pose, contact_probability, velocity, _ = self.forward_rnn(imu)
        # calculate pose (local joint rotation matrices)
        root_rotation = imu[:, -9:].view(-1, 3, 3)
        pose = self._reduced_glb_6d_to_full_local_mat(root_rotation.cpu(), global_reduced_pose.cpu()) # frame_no x 24 x 3 x 3
        ...
        j = art.math.forward_kinematics(pose[:, joint_set.lower_body],
                                        self.lower_body_bone.expand(pose.shape[0], -1, -1), # [9, 3] => [frame_no, 9, 3]
                                        joint_set.lower_body_parent)[1] 
                                        # forward_kinematics function takes local rotation, local position, and parent to output global rotations/positions
        tran_b1_vel = self.gravity_velocity + art.math.lerp(
            torch.cat((torch.zeros(1, 3, device=j.device), j[:-1, 7] - j[1:, 7])), # left foot
            torch.cat((torch.zeros(1, 3, device=j.device), j[:-1, 8] - j[1:, 8])), # right foot
            contact_probability.max(dim=1).indices.view(-1, 1).cpu()
        )
        tran_b2_vel = root_rotation.bmm(velocity.unsqueeze(-1)).squeeze(-1).cpu() * vel_scale / 60   # converts predicted velocity in root frame to global frame
        weight = self._prob_to_weight(contact_probability.cpu().max(dim=1).values.sigmoid()).view(-1, 1)
        velocity = art.math.lerp(tran_b2_vel, tran_b1_vel, weight) # equation from section 3.3.3
        # remove penetration
        current_root_y = 0
    ...
    @staticmethod
    def velocity_to_root_position(velocity: torch.Tensor):
        r"""
